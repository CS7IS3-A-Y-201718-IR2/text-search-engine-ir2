...
import com.ir.searchengine.query.inferencer.LDAInferencer; 
...
{ 
	...
	public Query expandQueryLDA(String queryStr, TopDocs hits, Properties prop) throws IOException {
		Vector<Document> vHits = getDocs(queryStr, hits, prop);
  ...
  */ 
	private void merge(Vector<TermQuery> terms) {
		for (int i = 0; i < terms.size(); i++) {
			...
			// Itterate through terms and if term is equal then merge: add the boost; and
			// delete the term
